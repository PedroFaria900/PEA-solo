<template>
  <div class="buy-page">
    ...
    <div class="bottom-action">
      <button class="btn-confirmar" @click="handlePagarClick" :disabled="isProcessing">
      ...
      <div v-if="showPassWarningModal" class="modal-overlay">
        <div class="modal-card">
          <div class="modal-top warning-bg" style="background: linear-gradient(135deg, #FFF3E0 0%, #FFE0B2 100%); padding-top: 44px; padding-bottom: 24px; text-align: center;">
            ...
            <h2 style="color: #15171A; font-size: 20px; font-weight: 700; margin: 0 0 8px 0; letter-spacing: -0.5px;">Passe equivalente a este bilhete</h2>
...
onMounted(async () => {
  try {
    ...
    const passes = res.data.filter(t => t.tipo === 'PASSE' && (t.estado === 'ATIVO' || t.estado === 'PENDENTE'))
    if (passes.length > 0) {
      activePass.value = passes[0]
    }
  } catch (err) {
    // ignore
  }
...
const irParaCarregarCarteira = () => {
  paymentStatus.value = null
  const missingAmount = Math.max(0, price.value - userBalance.value)
  if (missingAmount > 0) {
